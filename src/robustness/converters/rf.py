...
from robustness.models.psf import Constant, Variable, Not, And, Or
from robustness.models.types import (
    _RF_Type,
    _DT_Node_Type,
    _DT_Internal_Type,
    _Formula_Type,
...
def rf_to_formula_tree(rf: _RF_Type) -> _Formula_Type:
    conditions = [c for tree in rf for c in dt_to_formula_tree(tree)]
    if len(conditions) < 1:
...
def rf_to_formula_str(rf: _RF_Type) -> str:
    groups = [c for tree in rf for c in dt_to_formula_str(tree)]
